  ...
  return (
    <aside className={`bg-ui-panel border-r border-ui-border transition-all duration-200 ${isOpen ? "w-64" : "w-16"} overflow-hidden`}>
      <div className="p-3 flex flex-col gap-4 h-full">
        ...
        <button
          onClick={onCreate}
          className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium bg-primary text-primary-foreground hover:opacity-95"
        >
        ...
        <div className={`${isOpen ? "block" : "hidden"}`}>
          ...
          <div className="w-full h-36 rounded-xl border border-ui-border bg-ui-panel flex items-center justify-center text-sm text-muted">
            Mini month preview
          </div>
        </div>
